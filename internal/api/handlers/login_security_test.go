...
func TestLoginLockoutHTTP(t *testing.T) {
	...
	// First MaxLoginAttempts attempts should be rejected as unauthorized.
...
func TestLoginLockoutAfterMaxAttempts(t *testing.T) {
	...
	for i := 1; i <= MaxLoginAttempts; i++ {
		...
		if i < MaxLoginAttempts && locked {
			t.Fatalf("locked too early at attempt %d", i)
		}
		if i == MaxLoginAttempts && !locked {
			t.Fatalf("expected lockout after %d attempts", MaxLoginAttempts)
		}
	}
