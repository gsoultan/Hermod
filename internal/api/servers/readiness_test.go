...
func TestLivenessOK(t *testing.T) {
	s := api.NewServer(nil, &fakeStorage{}, nil, nil)
	handler := s.Routes()
...
func TestReadiness(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			s := api.NewServer(nil, &fakeStorage{listWorkflowsErr: tc.workflowsErr, listSourcesErr: tc.sourcesErr}, nil, nil)
			h := s.Routes()
...
func TestReadiness_Schema_And_NonGating(t *testing.T) {
	...
	s := api.NewServer(nil, fs, nil, nil)
