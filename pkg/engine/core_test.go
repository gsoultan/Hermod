...
func TestSinkWriter_PerKeyShardingOrder(t *testing.T) {
	...
	for i := range countPerKey {
		for _, k := range keys {
			...
			m.SetMetadata("key", k)
			pm := acquirePendingMessage(m)
			sw.enqueueWithStrategy(ctx, pm, config.BPBlock)
		}
	}
...
func TestBackpressure_DropNewest_Metric(t *testing.T) {
	...
	sw.enqueueWithStrategy(ctx, pm2, config.BPDropNewest)
	...
	val := testutil.ToFloat64(telemetry.BackpressureDropTotal.WithLabelValues(wf, sinkID, string(config.BPDropNewest)))
	if val < 1 {
...
func TestBackpressure_DropOldest_Metric(t *testing.T) {
	...
	m2.SetID("new")
	pm2 := acquirePendingMessage(m2)
	sw.enqueueWithStrategy(ctx, pm2, config.BPDropOldest)
	...
	val := testutil.ToFloat64(telemetry.BackpressureDropTotal.WithLabelValues(wf, sinkID, string(config.BPDropOldest)))
	if val < 1 {
...
func TestBackpressure_SpillToDisk_Metric(t *testing.T) {
	...
	sw := &sinkWriter{engine: e, sinkID: sinkID, config: config.SinkConfig{BackpressureStrategy: config.BPSpillToDisk}}
	...
	m2.SetID("spilled")
	pm2 := acquirePendingMessage(m2)
	sw.enqueueWithStrategy(ctx, pm2, config.BPSpillToDisk)
	...
	val := testutil.ToFloat64(telemetry.BackpressureSpillTotal.WithLabelValues(wf, sinkID))
	if val < 1 {
