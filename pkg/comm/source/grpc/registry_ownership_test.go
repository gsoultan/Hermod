...
func TestClosingTheSourceReleasesItsPath(t *testing.T) {
	const path = "/grpc/failover/via-source"

	s := NewGrpcSource(path)
...
func TestAClosedSourceDoesNotTakeOverAnotherOne(t *testing.T) {
	const path = "/grpc/failover/handover"

	outgoing := NewGrpcSource(path)
	incoming := NewGrpcSource(path)
