...
func TestTeardownStillRemovesItsOwnRegistration(t *testing.T) {
	const path = "/graphql/failover/solo"
...
func TestClosingTheSourceReleasesItsPath(t *testing.T) {
	const path = "/graphql/failover/via-source"

	s := NewGraphQLSource(path)
...
func TestAClosedSourceDoesNotTakeOverAnotherOne(t *testing.T) {
	const path = "/graphql/failover/handover"

	outgoing := NewGraphQLSource(path)
	incoming := NewGraphQLSource(path)
