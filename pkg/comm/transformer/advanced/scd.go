...
func init() {
	transformer.Register("scd", &SCDTransformer{})
}
...
func (t *SCDTransformer) handleType4(ctx context.Context, db *sql.DB, driver, table string, businessKeys, compareFields []string, config map[string]any, msg hermod.Message) (hermod.Message, error) {
	...
	historyTable := core.GetConfigString(config, "historyTable")
	if historyTable == "" {
...
func (t *SCDTransformer) handleType6(ctx context.Context, db *sql.DB, driver, table string, businessKeys []string, config map[string]any, msg hermod.Message) (hermod.Message, error) {
	...
	type1Cols := core.GetConfigStringSlice(config, "type1Columns")
	type2Cols := core.GetConfigStringSlice(config, "type2Columns")
	...
	startDateCol := core.GetConfigString(config, "startDateColumn")
	if startDateCol == "" {
		startDateCol = "start_date"
	}
	endDateCol := core.GetConfigString(config, "endDateColumn")
	if endDateCol == "" {
		endDateCol = "end_date"
	}
